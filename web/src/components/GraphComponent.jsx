...
export function GraphComponent({ publicView = false } = {}) {
    ...
    getNodesConfig(!publicView)
      .then((data) => {
        setNodeConfig((prev) => ({
          ...
          multiplierNodeSize:
            data.multiplier_node_size ?? defaultNodeConfig.multiplierNodeSize,
          nodeLabelsShow:
            data.node_labels_show ?? defaultNodeConfig.nodeLabelsShow,
        }));
      })
      .catch((e) => {
  ...
  const getSizeByCountNeighbors = useCallback(
    (node) => {
      return (
        (nodeNeighbors[node.id] || new Set()).size *
          nodeConfig.multiplierNodeSize || 1
      );
    },
      ...
      >
        <ForceGraph2D
          ...
          nodeCanvasObject={(node, ctx, globalScale) => {
            ...
            ctx.arc(
              node.x,
              node.y,
              nodeConfig.nodeRadius + sizeByCountNeighbors,
              0,
              2 * Math.PI,
              false
            );
            ...
            ctx.fillStyle = "#ddd";
            ctx.fillText(
              highlightNodes.has(node.id) || nodeConfig.nodeLabelsShow
                ? label
                : "",
              node.x + 10,
              node.y + 4
        ...
        >
          ...
          <Stack direction="row" gap={4} margin={2}>
            ...
            <Stack style={{ color: "#ccc", fontSize: 14 }}>
              <label htmlFor="nodeRadius">
                Размер узла: {nodeConfig.nodeRadius}px
              </label>
              <input
                type="range"
                id="nodeRadius"
                ...
                onChange={(e) =>
                  handleSetNodeConfig("nodeRadius", Number(e.target.value))
                }
                style={{ width: "100%", marginTop: 4 }}
              />
            </Stack>
            <Stack style={{ color: "#ccc", fontSize: 14 }}>
              <label htmlFor="nodeRadius" title="Влияет на узлы со связями">
                Множитель размера узла: {nodeConfig.multiplierNodeSize}
              </label>
              <input
                type="range"
            ...
            <Stack style={{ color: "#ccc", fontSize: 14 }}>
              <FormControlLabel
                onChange={(e, value) =>
                  handleSetNodeConfig("nodeLabelsShow", value)
                }
            ...
            <Stack
              style={{
                color: "#ccc",
                fontSize: 14,
                display: configIsChenged ? "block" : "none",
              }}
            >
              <Button
                variant="contained"
...
export function GraphSectionComponent({ publicView = false, sectionId } = {}) {
  const [windowSize, setWindowSize] = useState({
    width: window.innerWidth,
    height: window.innerHeight,
  });
        ...
        {graphData?.nodes?.length === 0 && (
          ...
          >
            ...
            <Button
              ...
              startIcon={<AddIcon />}
              onClick={() => navigate(`/management/sections/${sectionId}`)}
            >
              <Typography>Добавить узлы и связи</Typography>
            </Button>
          </Box>
        )}
        <Stack
